...
void draw(){
  ...
  for(int i = 0; i<po.length; i++){
    ...
    PVector va = new PVector(vx,vy);
    PVector vb = new PVector(xa,ya);
    
    va.sub(vb);
    va.mult(di*sin(3*t)+di/sin(12*t));
    
    vx = vx + coeffNow*random(-dr,dr)*di + va.x;
    vy = vy + coeffNow*random(-dr,dr)*di + va.y;
